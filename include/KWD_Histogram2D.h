...
constexpr auto KWD_VAL_BIPARTITE = "bipartite";
constexpr auto KWD_VAL_MINCOSTFLOW = "mincostflow";

// (fullmodel, colgen)
constexpr auto KWD_PAR_ALGORITHM = "Algorithm";
constexpr auto KWD_VAL_FULLMODEL = "fullmodel";
constexpr auto KWD_VAL_COLGEN = "colgen";

// ('SILENT', 'INFO', 'DEBUG')
constexpr auto KWD_PAR_VERBOSITY = "Verbosity";
constexpr auto KWD_VAL_SILENT = "silent";
constexpr auto KWD_VAL_INFO = "info";
constexpr auto KWD_VAL_DEBUG = "debug";
...
namespace KWD {
	...
	public:
		// Standard c'tor
		Solver()
			: _runtime(0.0), _n_log(0), L(-1), recode(""), opt_tolerance(1e-06),
		...
		// In case of coordinates with regulare distance, recode the coordinate to conescutive integers
		...
		double compareExact(int _n, int* _Xs, int* _Ys, double* _W1, double* _W2) {
			...
			if (check_coding(_n, _Xs))
				PRINT("WARNING: the Xs input coordinates are not consecutives integers.\n");
			if (check_coding(_n, _Ys))
				PRINT("WARNING: the Ys input coordinates are not consecutives integers.\n");
			...
			for (int i = 0; i < _n; i++) {
				...
				if (_W1[i] < 0.0) {
					PRINT("WARNING: weight W1[%d]=%.4f is negative. Only positive weights are allowed.\n", i, _W1[i]);
					throw std::runtime_error("FATAL ERROR: Input histogram with negative weigths");
				}
				if (_W2[i] < 0.0) {
					PRINT("WARNING: weight W2[%d]=%.4f is negative. Only positive weights are allowed.\n", i, _W2[i]);
					throw std::runtime_error("FATAL ERROR: Input histogram with negative weigths");
				}
			}
		...
		vector<double> compareApprox(int _n, int _m, int* _Xs, int* _Ys, double* _W1,
			...
			for (int i = 0; i < _n; i++) {
				...
				for (int j = 0; j < _m; ++j) {
					...
					tot_ws[j] += _Ws[j * _n + i];
					if (_Ws[j * _n + i] < 0.0) {
						PRINT("WARNING: weight Ws[%d,%d]=%.4f is negative. Only positive weights are allowed.\n", i, j, _Ws[j * _n + i]);
						throw std::runtime_error("FATAL ERROR: Input histogram with negative weigths");
					}
				}
			}
