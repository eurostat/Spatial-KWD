...
\details{
  ...
  \itemize{
    ...
    \item \code{compareApprox(Xs, Ys, Ws, L)}: compute a symmetric matrix of pairwise approximate distances (which quality depends on the value of $L$) between all the possible pairs of the vector listed in \code{Ws}. 
    The algorithm used by the solver is controlled by the parameter \code{ApproxMethod} (see below).
  ...
  \itemize{
    \item \code{ExactMethod}: set which algorithm to use to compute the exact distance between a pair of histograms. The options for this parameter are: 
      ...
      \item \code{mincostflow}: it uses a complete uncapacitated network flow, corresponding to use an approximate method with the parameter $L=N-1$. This may be faster than the \code{bipartite} method, depending on the type of histograms.
      
      \item \code{colgen}: it use a column generation algorithm on the same network used by \code{mincostflow}. However, the arcs of the network are added only when they are violated. Moreover, the arcs of the network are removed as soon as they slack is strictly positive. This permits to handle only a number of variables of size $O(n)$, where $n$ is the number of non-empty bins.
      }
      The default value is set to \code{colgen}.

    \item \code{ApproxMethod}: set which algorithm to use to compute an approximate distance between a pair of histograms, which depends on the parameter $L$. The options for this parameter are: 
      \itemize{
      \item \code{mincostflow}: it uses an uncapacitated network flow which size depends on the input parameter $L$. Larger value of $L$ gives more precise distances, but at the expense of higher running times.

      \item \code{colgen}: it uses an uncapacitated network flow which size depends on the input parameter $L$, but where the arc variables are added and removed dynamically, in order to keep the core problem as small as possible. It is the recommended method for very large spatial maps. On medium and small spatial maps the previous method could be faster.
    }
      The default value is set to \code{colgen}.
